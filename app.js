...
//GETTING ARRAYS OF USER FOLLOWING PPL ID's:-

const getFollowingPplIdsOfUser = async (username) => {
  const getFollowingPplQuery = `SELECT following_user_id FROM follower
    INNER JOIN user ON user.user_id = follower.follower_user_id 
    WHERE user.username = '${username}';`;

  const followingPplResult = await db.all(getFollowingPplQuery);
  const arrayOfIds = followingPplResult.map(
...
app.get("/user/tweets/feed/", authentication, async (request, response) => {
  ...
  const followingPplIds = await getFollowingPplIdsOfUser(username);

  const getTweetsQuery = `SELECT username, tweet, date_time as dateTime
    FROM user INNER JOIN tweet ON user.user_id = tweet.user_id 
    WHERE user.user_id IN (${followingPplIds}) ORDER BY date_time DESC LIMIT 4;`;

  const tweetsResult = await db.all(getTweetsQuery);
...
app.get("/user/following/", authentication, async (request, response) => {
  ...
  const getFollowingUsersQuery = `SELECT name FROM follower INNER JOIN 
    user ON follower.following_user_id = user.user_id WHERE 
...
app.get(
  ...
  async (request, response) => {
    ...
    const repliesUsers = await db.all(getRepliesQuery);
...
app.get("/user/tweets/", authentication, async (request, response) => {
  ...
  const getTweetsQuery = `SELECT tweet, COUNT(DISTINCT like_id) AS likes,
    ...
    LEFT JOIN like ON tweet.tweet_id = like_tweet_id
...
app.post("/user/tweets/", authentication, async (request, response) => {
  ...
  const createTweetQuery = `INSERT INTO tweet
    (tweet,user_id,date_time) VALUES ('${tweet}','${userId}','${dateTime}');`;
